...
//функция, объединяющая ФИО
function getFullnameFromParts ($surname, $name, $patronymic){
    return $surname .' '. $name .' '. $patronymic;
} 

// //функция, разделяющая ФИО
...
function getShortName ($arr) {
    ...
    $name = $separateFullName['name']; // получаем имя
    $shortSurname = mb_substr($separateFullName['surname'], 0, 1, "UTF-8"); // получаем первую букву фамилии
...
function getPerfectPartner ($surname, $name, $patronymic, $arr) {
    ...
    // случайным образом выбираем человека в массиве
    do {
...
$separateFullName = getPartsFromFullname("Иванов Иван Иванович"); // разделяем ФИО на фамилию, имя и отчество
...
$fullName = getFullnameFromParts ($surname, $name, $patronymic); // соединяем ФИО в одну строку 
echo("Объединенное ФИО: <br>");
print_r($fullName);
echo "<br>";
echo "<br>";

$shortName = getShortName("Иванов Иван Иванович"); // сокращаем ФИО
echo("Сокращенное ФИО: <br>");
print_r($shortName);
echo "<br>";
echo "<br>";

$gender = getGenderFromName("Иванов Иван Иванович"); // определяем пол
echo("Пол: <br>");
print_r($gender);
echo "<br>";
echo "<br>";

getGenderDescription ($example_persons_array); // определяем половой состав аудитории
echo "<br>";
echo "<br>";

getPerfectPartner('Иванов', 'Иван', 'Иванович', $example_persons_array); // проверяем совместимость мужчины

echo "<br>";

getPerfectPartner('Петрова', 'Анна', 'Сергеевна', $example_persons_array); // проверяем совместимость женщины
?>